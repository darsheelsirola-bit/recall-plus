...
export const CBSE_2026_27_XI_PATHWAY_PRESETS: Readonly<Record<
  AcademicPathway,
...
export function subjectIdsForPreset(
  pathway: AcademicPathway,
  preset: string,
): readonly string[] {
  const codes = CBSE_2026_27_XI_PATHWAY_PRESETS[pathway]?.[preset] || []
...
export function validateCbse2026ClassXiCombination(
  ...
  const resolved = normalizedSelections.map((selection) => ({
    selection,
    subject: CBSE_2026_27_XI_SUBJECTS_BY_ID.get(selection.curriculumSubjectId),
  }))
  resolved.forEach(({ subject }) => {
    if (!subject) {
      errors.push(error('UNKNOWN_SUBJECT', 'One or more selected subjects are not part of the current CBSE Class XI catalogue.'))
  ...
  if (
    subjectOne
    && (
      subjectOne.subjectGroup !== CBSE_2026_27_XI_RULES.subjectOne.allowedGroup
      || !CBSE_2026_27_XI_RULES.subjectOne.allowedCodes.includes(
        subjectOne.subjectCode as typeof CBSE_2026_27_XI_RULES.subjectOne.allowedCodes[number],
      )
    )
  ) {
    errors.push(error(
  ...
  if (
    subjectTwo
    && !CBSE_2026_27_XI_RULES.subjectTwoAllowedGroups.includes(
  ...
  for (let position = 3; position <= 4; position += 1) {
    ...
    if (
      subject
      && !CBSE_2026_27_XI_RULES.subjectThreeToFourAllowedGroups.includes(
  ...
  if (
    subjectFive
    && subjectFive.subjectGroup !== CBSE_2026_27_XI_RULES.subjectFiveAllowedGroup
  ) {
    errors.push(error(
      'SUBJECT_FIVE_GROUP',
  ...
  if (
    subjectSix
    && !CBSE_2026_27_XI_RULES.subjectSixAllowedGroups.includes(

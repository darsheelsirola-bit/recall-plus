...
export default function StudyLogList({ logs, onEdit, emptyHint = 'Add your first study log to see it here.' }) {
  if (!logs.length) {
    return (
  ...
  return (
    <div className="flex max-h-[60vh] flex-col gap-3 overflow-y-auto pr-1">
      {logs.map((log) => {
        ...
        return (
          <article key={log.id} className="rounded-xl border border-border bg-background p-4 transition hover:border-primary/25">
            <div className="flex items-start justify-between gap-3">
              ...
              <div className="flex shrink-0 items-center gap-2">
                ...
                {onEdit ? <Button type="button" onClick={() => onEdit(log)} variant="ghost" size="icon-sm" aria-label={`Edit ${log.subject} log`}><Pencil /></Button> : null}
              </div>
            </div>

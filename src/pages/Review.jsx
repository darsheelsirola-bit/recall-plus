...
export default function Review() {
  ...
  return (
    <>
      ...
      <div className="flex gap-3 overflow-x-auto border-b border-ink/10 pb-3">{tabs.map((item) => <button key={item} className={`tab-button ${tab === item ? 'tab-button-active' : ''}`} onClick={() => setTab(item)}>{item}<span className="ml-2 opacity-55">{groups[item].length}</span></button>)}</div>

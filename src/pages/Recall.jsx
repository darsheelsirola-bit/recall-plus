        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {queue.map((item) => (
            <Card key={item.id}>
              ...
              <CardFooter className="gap-3 bg-transparent"><Button variant="ghost" className="flex-1" onClick={() => markRevised(item.id)}><CheckCircle2 data-icon="inline-start" /> Revised</Button><Button className="flex-1" render={<Link to={quizLink(item)} />}>Test me <ArrowRight data-icon="inline-end" /></Button></CardFooter>
            </Card>
          ))}
        </div>
      ) : (
        <>
          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {suggestions.map((item) => (
              <Card key={`${item.subject}-${item.chapter}-${item.topic}`}><CardHeader><span className="grid size-10 place-items-center rounded-xl bg-secondary text-primary"><BookOpen className="size-4" /></span><Badge variant="outline" className="mt-3 w-fit rounded-full">{item.subject}</Badge><CardTitle className="mt-2 text-lg">{item.topic}</CardTitle><CardDescription>{item.chapter}</CardDescription></CardHeader><CardFooter className="gap-3 bg-transparent"><Button variant="outline" className="flex-1" render={<Link to={`/add-log?subject=${encodeURIComponent(item.subject)}&chapter=${encodeURIComponent(item.chapter)}&topic=${encodeURIComponent(item.topic)}`} />}>Log study</Button><Button className="flex-1" render={<Link to={quizLink(item)} />}>Small quiz</Button></CardFooter></Card>
            ))}
